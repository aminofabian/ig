...
import { Search, Loader2, Hash, Heart, MessageCircle, BookmarkPlus, Download } from 'lucide-react';
...
export default function HashtagSearch() {
  ...
  // Basic search function - fetches first page of results
  const searchHashtag = async (term: string) => {
    ...
    try {
      ...
      if (result.pagination) {
        setNextPageToken(result.pagination.next_page_token || '');
        setHasMorePosts(result.pagination.has_next_page || false);
      }
  ...
  // Load more posts when user clicks "Load More"
  const loadMorePosts = async () => {
    if (!searchQuery || !nextPageToken) return;
    
    try {
      const nextPage = currentPage + 1;
      const response = await fetch(`/api/instagram/hashtag?hashtag=${encodeURIComponent(searchQuery)}&page=${nextPage}&next_page_token=${nextPageToken}`);
      ...
      const errorMessage = err instanceof Error ? err.message : 'An unexpected error occurred';
      toast.error(`Failed to load more posts: ${errorMessage}`);
    }
  };

  // Function to fetch all posts for the hashtag using server action
  const fetchAllPosts = async () => {
    if (!searchQuery) {
      toast.error("No hashtag selected");
      return;
    }
    ...
    try {
      toast.info(`Fetching all posts for #${searchQuery}, this may take a moment...`);
      
      // Call the server action to fetch all posts
      const response = await fetch(`/api/instagram/fetch-all?hashtag=${encodeURIComponent(searchQuery)}`);
      ...
      if (allPostsData.error) {
        throw new Error(allPostsData.error);
      }
      
      console.log(`Fetched all ${allPostsData.total_count} posts for #${searchQuery}`);
      
      // Update hashtagData with all items
      if (allPostsData.items?.length) {
        setHashtagData(prev => {
          if (!prev) return { items: allPostsData.items };
          
          return {
            ...prev,
            items: allPostsData.items
          };
        });
        
        toast.success(`Successfully loaded ${allPostsData.total_count} posts for #${searchQuery}`);
        setHasMorePosts(false); // No more posts to load
      } else {
        toast.info("No additional posts found");
      }
                ...
                <div className="flex items-center gap-6 mb-6">
                    ...
                    <div className="mt-3 flex gap-4">
                      <div className="text-white">
                        <span className="text-lg font-bold">
                          {hashtagData.additional_data?.formatted_media_count || formatCount(hashtagData.additional_data?.media_count || 0)}
                        </span>
                        <span className="text-gray-400 ml-2">posts</span>
                      </div>
                      <div className="text-white">
                        <span className="text-lg font-bold">{formatCount(avgLikes)}</span>
                        <span className="text-gray-400 ml-2">avg likes</span>
                      </div>
                      <div className="text-white">
                        <span className="text-lg font-bold">{formatCount(avgComments)}</span>
                        <span className="text-gray-400 ml-2">avg comments</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={saveHashtag}
                      disabled={isSaving}
                      className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[#ee46c7] to-[#f059da] text-white rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity duration-200 shadow-md"
                    >
                      {isSaving ? (
                    ...
                    <button
                      ...
                      disabled={loadingAllPosts || !searchQuery}
                      className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity duration-200 shadow-md"
                    >
                      {loadingAllPosts ? (
                        <Loader2 className="w-4 h-4 text-white animate-spin" />
                      ) : (
                      ...
                      <span className="hidden md:inline">Fetch All</span>
                    </button>
                  </div>
                </div>
              </div>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {currentPosts.map((post, index) => (
                      <div key={post.id || index} className="relative bg-black/30 border border-white/5 rounded-lg overflow-hidden shadow-lg group hover:border-[#f059da]/50 transition-all duration-300">
                        {post.thumbnail_url && (
                            ...
                            {post.media_type === 2 && (
                              <div className="absolute top-2 right-2 bg-black/70 rounded-full p-1">
                                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                  <path d="M6.5 17.5L6.5 2.5L17.5 10L6.5 17.5Z" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="p-4">
                          <div className="flex items-center justify-between mb-3">
                            {post.user && (
                              <div className="flex items-center gap-2">
                                {post.user.profile_pic_url && (
                                  <img
                                    src={getProxiedImageUrl(post.user.profile_pic_url)}
                            ...
                            {post.taken_at && (
                              <span className="text-xs text-gray-400">{formatDate(post.taken_at)}</span>
                            )}
                          </div>
                          <div className="text-sm text-white/80 max-h-16 overflow-hidden mb-3">
                            {formatCaption(post.caption_text)}
                          </div>
                  ...
                  <div className="flex justify-between items-center py-4">
                    <div className="text-white/70 text-sm">
                      Showing {currentPosts.length} of {hashtagData.items?.length || 0} posts
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => handlePageChange(currentPage - 1)}
                        disabled={currentPage === 1}
                        className="px-3 py-1 rounded bg-black/50 text-white disabled:opacity-30"
                      >
                        Previous
                      </button>
                  ...
                  {hasMorePosts && (
                    <div className="flex justify-center mt-6">
                      <button
                        ...
                        className="px-6 py-2 bg-[#f059da]/20 hover:bg-[#f059da]/40 text-white rounded-lg border border-[#f059da]/30 transition-colors duration-200"
                      >
                        Load More Posts
                      </button>
                    </div>
