...
function AdminPage() {
  ...
  const mappedUsers: User[] = users.map((user: any) => ({
    ...
    status: 'Active',
    subscription: {
      status: user.subscription?.status || SUBSCRIPTION_STATUS.INCOMPLETE,
  ...
  const handleUpdateSubscription = async (userId: string, subscriptionStatus: SUBSCRIPTION_STATUS) => {
    try {
      const response = await fetch('/api/users/update', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        ...
        <Dialog open={isSubscriptionModalOpen} onOpenChange={setIsSubscriptionModalOpen}>
          <DialogContent className="bg-zinc-900/95 border-white/20 backdrop-blur-xl">
            <DialogHeader>
              ...
              <DialogDescription className="text-zinc-400">
    {selectedUser?.subscription?.status === SUBSCRIPTION_STATUS.ACTIVE
